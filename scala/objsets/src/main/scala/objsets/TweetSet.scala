...
abstract class TweetSet {
   ...
   */
  def filter(p: Tweet => Boolean): TweetSet = filter0(p, new Empty)		// BH -> define the start of the filter with Empty Set
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet

  def union(that: TweetSet): TweetSet = if (isEmpty) that else (this.tail union that) incl head	// BH -> similar to IntSets example

  // Hint: the method "remove" on TweetSet will be very useful.
  def ascendingByRetweet: Trending = 
  {
    def ascending(set:TweetSet, sequence:Trending): Trending =
...
class Empty extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = accu			// BH -> end of the line, just return the accumulated set
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = 
    // BH -> merge the left filtered with the right filtered, which also gets the root elem included if needed
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet =	// BH -> over the master Tweet set return all those that have a text matching a google string
    TweetReader.allTweets.filter (t=>google.exists(s=>t.toString.contains(s)))

  val appleTweets: TweetSet = 
    TweetReader.allTweets.filter (t=>apple.exists(s=>t.toString.contains(s)))
