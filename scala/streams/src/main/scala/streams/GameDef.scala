...
trait GameDef {
   ...
   * the game.
   */
  def startBlock: Block = Block (startPos,startPos)					// both parts of the block are at the starting position
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = 
      List( (left,Left), (right,Right), (up,Up), (down,Down) )		// just write them all out in any order
    ...
    def legalNeighbors: List[(Block, Move)] =
      neighbors filter { case(block,_)=>block.isLegal }				// remove all moves which result in illegal blocks

    /**
     * Returns `true` if the block is standing.
     */
    def isStanding: Boolean =
      b1.x==b2.x && b1.y==b2.y										// both parts of the block are at the same location

    /**
     * Returns `true` if the block is entirely inside the terrain.
     */
    def isLegal: Boolean =
      terrain(b1) && terrain(b2)									// both parts of the block are at valid locations
