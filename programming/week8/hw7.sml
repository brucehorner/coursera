...
(* CHANGE: Add a case for Shift expressions *)
	  ...
	  | Intersect (es1,es2) => eval_prog (Shift(dx, dy, eval_prog(e1,env)), env)
	  | Let (s,es1,es2) => eval_prog (Shift(dx, dy, eval_prog(e1,env)), env)
	  | Var (s) => eval_prog (Shift(dx, dy, eval_prog(e1,env)), env)
	 (* | Shift (dsx,dsy,es1) => e1 *) (* TO DO *)
...
fun preprocess_prog (e) =
	...
	then Point(x1,y1)
		...
		else e (* no need to change *)
	    else
		if (x2 < x1) then LineSegment (x2,y2,x1,y1)
		else e (* no need to change *)
      | Intersect(es1,es2) =>
	Intersect (preprocess_prog(es1), preprocess_prog(es2))
      | Let (s,es1,es2) =>
	Let (s, preprocess_prog(es1), preprocess_prog(es2))
      | Shift (dx,dy,es1) => Shift (dx, dy, preprocess_prog(es1))
